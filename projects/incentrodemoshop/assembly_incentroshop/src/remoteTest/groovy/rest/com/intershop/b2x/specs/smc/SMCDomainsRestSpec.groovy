...
class SMCDomainsRestSpec extends RestSpec
{
    ...
    def setupSpec()
    {
        setup:
            ...
            restServiceUri = testData.get("rest.smc.uri.ish")[0] + "/domains"
    }

    @Unroll
    def "Check Admin user sees Domains resource using contentType #contentType"()
    {
        given:
            client.setAuthorizationHeader(adminUserLogin, adminUserPassword)
            client.headers['Accept'] = contentType
        when: "request GET Domains"
